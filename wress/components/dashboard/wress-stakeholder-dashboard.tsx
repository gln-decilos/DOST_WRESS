...
function getReadableActions(project: ProjectDashboard | undefined) {
  ...
  if (permissions.includes("requirements.create") || permissions.includes("requirements.edit")) {
    actions.push("Work on requirements")
  }

  if (permissions.includes("requirements.approve") || permissions.includes("requirements.reject")) {
  ...
  if (permissions.includes("vision_scope.create") || permissions.includes("vision_scope.edit")) {
...
function getAllDocuments(project: ProjectDashboard | undefined) {
  ...
  const documents: DashboardDocument[] = [
    ...project.requirementDocuments.map((document) => ({
      ...
      title: document.name || `Requirements Document ${document.version || ""}`.trim(),
    })),
...
function countByStatus(documents: DashboardDocument[], status: string) {
  return documents.filter((document) => normalizeStatus(document.status) === status).length
}
...
function buildHelpfulInsights({
  ...
  const draftCount = showProjectTeamView ? countByStatus(allDocuments, "draft") : 0
  ...
  const hasRequirementDocument = allDocuments.some((document) => document.type === "Requirements")
  const hasVisionDocument = allDocuments.some((document) => document.type === "Vision and Scope")
  ...
  if (showProjectTeamView) {
    ...
    if (reviewCount > 0) {
      insights.push({
        ...
        message:
          `${reviewCount} document${reviewCount > 1 ? "s are" : " is"} waiting for review. Follow up with the assigned reviewers to avoid delay.`,
      })
    }
    ...
    if (rejectedCount > 0) {
      insights.push({
        ...
        message:
          `${rejectedCount} document${rejectedCount > 1 ? "s have" : " has"} been rejected. Check the comments and revise before sending again.`,
      })
    }
    ...
    if (draftCount > 0) {
      insights.push({
        ...
        message:
          `${draftCount} draft document${draftCount > 1 ? "s are" : " is"} still in progress. Check if any draft is ready to send for review.`,
      })
    }
    ...
    if (requirementCount > 0 && !reviewCount && !draftCount) {
      insights.push({
        ...
        message:
          `${requirementCount} requirement item${requirementCount > 1 ? "s are" : " is"} recorded in this project. Continue checking changes before approval or locking.`,
      })
    }
    ...
    if (frozenCount > 0) {
      insights.push({
        ...
        message:
          `${frozenCount} frozen document${frozenCount > 1 ? "s are" : " is"} available. Use frozen documents as the stable reference for project work.`,
      })
    }
    ...
    if (staleDays !== null && staleDays >= 14) {
      insights.push({
        ...
        message:
          `The latest visible document update was ${staleDays} days ago. Check if the documents still match the current project status.`,
      })
    }
    ...
    if (daysUntilEnd !== null && daysUntilEnd < 0 && (reviewCount > 0 || draftCount > 0)) {
      insights.push({
        ...
        message:
          "The project end date has passed, but there are still pending documents. Review the schedule and update the project plan.",
      })
      ...
      insights.push({
        ...
        message:
          `The project end date is in ${daysUntilEnd} day${daysUntilEnd === 1 ? "" : "s"}. Check pending reviews and documents before the deadline.`,
      })
    }
  ...
  if (showReviewView && reviewCount > 0) {
    insights.push({
      ...
      message:
        `${reviewCount} document${reviewCount > 1 ? "s are" : " is"} waiting for your review. Open the document and approve or reject it based on the project needs.`,
    })
  }
  ...
  if (rejectedCount > 0) {
    insights.push({
      ...
      message:
        "A rejected document is visible in this project. You may check it for context, but the project team should handle the revision.",
    })
  }
  ...
  if (approvedCount > 0) {
    insights.push({
      ...
      message:
        `${approvedCount} approved or frozen document${approvedCount > 1 ? "s are" : " is"} available. You can use this as your current reference for the project.`,
    })
  }
  ...
  if (!visibleDocuments.length) {
    insights.push({
      ...
      message:
        "There are no documents available for your role in this project. Wait for the project team to share a document or ask for an update.",
    })
  }
  ...
  if (staleDays !== null && staleDays >= 14) {
    insights.push({
      ...
      message:
        `The latest visible document update was ${staleDays} days ago. Ask the project team if there are new changes you should know about.`,
    })
  }
  ...
  if (daysUntilEnd !== null && daysUntilEnd <= 7 && daysUntilEnd >= 0) {
    insights.push({
      ...
      title: "Project end date is near",
      message:
        `The project end date is in ${daysUntilEnd} day${daysUntilEnd === 1 ? "" : "s"}. Check if any document still needs your review.`,
    })
  }
...
async function buildProjectDashboard(
  ...
  const roles = profileProject?.roles?.map((role) => role.role_name).filter(Boolean) || []
  ...
  return {
    ...
    requirementDocuments: Array.isArray(requirementsData?.documents)
      ...
      : [],
    visionDocuments: Array.isArray(visionData?.documents) ? visionData.documents : [],
  }
}
...
export function WressStakeholderDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      const profileProjects = Array.isArray(profileData?.projects) ? profileData.projects : []
      const profileMap = new Map(profileProjects.map((project) => [project.project_id, project]))
      const assignedProjectIds = new Set(profileProjects.map((project) => project.project_id))
      ...
      const dashboards = await Promise.all(
        visibleProjects.map((project) => buildProjectDashboard(project, profileMap.get(project.id))),
      )
  ...
  const allSelectedDocuments = useMemo(() => {
    return getAllDocuments(selectedProject)
  }, [selectedProject])
  ...
  const analytics = useMemo(() => {
    ...
    const draftCount = showProjectTeamView ? countByStatus(allSelectedDocuments, "draft") : 0
    ...
    <div className="space-y-3">
      <section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
        <div className="grid gap-3 p-3 md:grid-cols-[minmax(0,1fr)_minmax(240px,320px)] md:items-center">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="mb-1.5 inline-flex items-center gap-2 rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary">
              ...
              <p className="mt-0.5 max-w-2xl text-xs text-muted-foreground">
                Select a project to see useful insights based on your assigned role.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border bg-muted/50 p-2.5">
            ...
            <select
              ...
              className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-xs font-medium text-foreground outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-60"
            >
      ...
      {loadState === "offline" && (
        <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">
          ...
          <p>
            The dashboard could not connect to the backend. Start the Flask server and refresh this
            page to see live project data.
          </p>
        </div>
      )}
      ...
      {showProjectTeamView ? (
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            note="Projects where you have a role"
          />
          ...
          <StatCard
            icon={ClipboardCheck}
            label="Requirements"
            value={analytics.requirementCount}
            note="Requirement items in this project"
          />
          ...
          <StatCard
            ...
            note="Documents waiting for decisions"
          />
        </section>
      ) : (
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            label="Assigned projects"
            value={projectDashboards.length}
            note="Projects shared with you"
          />

          <StatCard
            icon={ShieldCheck}
            label="Your role"
            value={selectedProject?.roles[0] || "Stakeholder"}
            note="Role in the selected project"
          />
          ...
          <StatCard
            ...
            value={analytics.reviewCount}
            note="Documents that need your review"
          />
      ...
      <section className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(360px,500px)]">
              ...
              <>
                <div className="grid gap-2 sm:grid-cols-3">
                  ...
                  <div className="rounded-xl border bg-background p-3">
                    ...
                    <p className="mt-1 text-base font-semibold text-foreground">
                      {analytics.latestDate ? formatDate(analytics.latestDate) : "No update"}
                    </p>
                  </div>
                ...
                {showProjectTeamView && (
                  <div className="rounded-xl border bg-background p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-medium text-foreground">Document readiness</p>
                        <p className="text-xs text-muted-foreground">
                          Approved or frozen documents compared with visible documents
                        </p>
                      </div>

                      <p className="text-xl font-bold text-foreground">{analytics.donePercent}%</p>
                    </div>

                    <Progress value={analytics.donePercent} className="mt-3" />
                  </div>
                )}
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                  {showProjectTeamView && (
                    <div className="rounded-xl border bg-background p-3">
                      <p className="text-xl font-bold text-foreground">{analytics.draftCount}</p>
                      <p className="text-xs text-muted-foreground">Draft documents</p>
                    </div>
                  )}

                  <div className="rounded-xl border bg-background p-3">
                    <p className="text-xl font-bold text-foreground">{analytics.reviewCount}</p>
                    <p className="text-xs text-muted-foreground">
                      {showProjectTeamView ? "Needs review" : "Waiting for your review"}
                    </p>
                  </div>

                  <div className="rounded-xl border bg-background p-3">
                    <p className="text-xl font-bold text-foreground">{analytics.approvedCount}</p>
                    <p className="text-xs text-muted-foreground">Approved or frozen</p>
                  </div>

                  <div className="rounded-xl border bg-background p-3">
                    <p className="text-xl font-bold text-foreground">{analytics.rejectedCount}</p>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  {canSeeDocuments(selectedProject) ? (
                    <Button asChild className="h-9 justify-start">
        ...
        <Card className="w-full shadow-sm xl:justify-self-end">
          <CardHeader className="px-4 pb-2 pt-4">
            <CardTitle>Projects and roles</CardTitle>
            <CardDescription>Click a project to change the dashboard view</CardDescription>
          </CardHeader>
          ...
          <CardContent className="px-4 pb-4 pt-0">
            {projectDashboards.length ? (
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="px-3 py-2">Roles</TableHead>
                  ...
                  <TableBody>
                    {projectDashboards.map((projectItem) => {
                      const actions = getReadableActions(projectItem)
                      const isSelected = selectedProject?.project.id === projectItem.project.id
                      const visibleDocuments = getVisibleDocuments(projectItem)
                      ...
                      return (
                        ...
                        >
                          <TableCell className="px-3 py-2 align-top">
                            <button
                              type="button"
                              onClick={() => setSelectedProjectId(projectItem.project.id)}
                              className="w-full text-left transition hover:text-primary"
                            >
                              <p className="truncate font-medium text-foreground">
                                {projectItem.project.name}
                              </p>
                              <p className="text-xs text-muted-foreground">
                                {getProjectStatusLabel(projectItem.project.status)}
                              </p>
                            </button>
                          </TableCell>
                          ...
                          <TableCell className="px-3 py-2 align-top">
                            <div className="flex flex-wrap gap-1">
                              {(projectItem.roles.length ? projectItem.roles : ["No role yet"]).map(
                                (role) => (
                          ...
                          <TableCell className="px-3 py-2 align-top">
                            <div className="flex flex-wrap gap-1">
                              ...
                              {actions.length > 2 && (
                                <Badge variant="outline">+{actions.length - 2}</Badge>
                              )}
                            </div>
                          </TableCell>

                          <TableCell className="px-3 py-2 text-right align-top font-medium">
              ...
              <EmptyState
                title={loadState === "loading" ? "Loading projects" : "No assigned projects"}
