...
export default function RequirementsDetailsPageView({
  ...
  const [linkOptionMap, setLinkOptionMap] = useState<Record<string, string>>({})

  const getFieldValueFromDocument = (fieldId: number) => {
    const value = documentState?.values?.find((item) => item.template_field_id === fieldId)
    return value?.value_text || ""
  }

  const getDisplayValue = (field: TemplateField) => {
    const rawValue = getFieldValueFromDocument(field.id)

    if (!rawValue) return "-"

    if (field.key === "linked_requirement") {
      return linkOptionMap[rawValue] || rawValue
    }
  ...
  const fetchData = async () => {
    try {
      ...
      const [requirementRes, linkOptionsRes] = await Promise.all([
        ...
        fetch(
          `${API_BASE_URL}/project/${projectId}/requirements/link-options`,
          {
            method: "GET",
            credentials: "include",
          }
      ...
      const options = (linkOptionsData.options || []) as LinkOption[]
      ...
      setLinkOptionMap(optionMap)
      setTemplate(requirementData.template)
      ...
      requirementData.template.sections.forEach((section) => {
  ...
  if (!template || !requirement || !documentState) {
    return (
      <section className="rounded-2xl bg-card p-6 shadow-sm ring-1 ring-border md:p-8">
        <div className="rounded-2xl bg-background p-8 text-center text-muted-foreground ring-1 ring-border">
  ...
  return (
    <section className="w-full rounded-2xl bg-card p-6 shadow-sm ring-1 ring-border md:p-8">
      <div className="mb-6">
        <button
          onClick={() => {
            window.location.href = `/project/${projectId}?tab=requirements`
          }}
