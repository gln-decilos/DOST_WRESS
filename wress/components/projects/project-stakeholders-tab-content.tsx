...
type ProjectStakeholdersTabContentProps = {
  projectId: number
}
...
export default function ProjectStakeholdersTabContent({
  projectId,
}: ProjectStakeholdersTabContentProps) {
            ...
            <tbody>
              {fetching ? (
                ...
                stakeholders.map((stakeholder) => (
                  <tr key={stakeholder.id} className="border-t border-border">
                    <td className="px-4 py-3 font-medium text-foreground">
                      {stakeholder.user?.full_name ||
                        `${stakeholder.user?.first_name || ""} ${stakeholder.user?.last_name || ""}`.trim() ||
                        "Unknown User"}
                    </td>
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              ...
              <span className="font-medium text-foreground">
                {stakeholderToDelete.user?.full_name ||
                  `${stakeholderToDelete.user?.first_name || ""} ${stakeholderToDelete.user?.last_name || ""}`.trim() ||
                  "this stakeholder"}
              </span>{" "}
              from this project?
            </p>
