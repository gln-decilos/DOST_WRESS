...
const API_BASE_URL = "http://localhost:5000/api/admin/organizations/"
...
export default function OrganizationsPageView() {
  ...
  const fetchOrganizations = async () => {
    try {
      ...
      if (!res.ok) {
        console.error("Failed to fetch organizations:", res.status, text)
      ...
      if (!data) {
        console.error("Invalid JSON response for organizations:", text)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = selectedOrganization.id
        ? `${API_BASE_URL}${selectedOrganization.id}`
        : API_BASE_URL
      ...
      if (!data && text) {
        console.error("Invalid JSON response while saving organization:", text)
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}${organizationToDelete.id}`, {
        method: "DELETE",
      })
      ...
      if (!data && text) {
        console.error("Invalid JSON response while deleting organization:", text)
        setMessage("Server returned an invalid response")
        return
      }

      if (!res.ok) {
        setMessage(data?.message || data?.error || "Failed to delete organization")
        return
      }
