...
export default function UsersPage() {
  ...
  const fetchOrganizations = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/users/organizations`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  useEffect(() => {
    if (userId && user && user.user_type === "System Admin") {
      fetchUsers()
      fetchOrganizations()
    } else if (userId && user && user.user_type !== "System Admin") {
      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            Manage user accounts, user types, organizations, and status.
          </p>
        </div>
                      ...
                      <label key={organization.id} className="flex items-center gap-2 text-sm text-foreground">
                        <input
                          ...
                          checked={
                            ...
                            selectedUser.organizations[0].id === organization.id
                          }
                          onChange={() =>
                            setSelectedUser((prev) => ({
                              ...prev,
